...
describe.skip('type tests', () => {
  ...
  mongoose.connect(connectUri).then(cb).then(null, cb);
  ...
  conn1.openSet('mongodb://localhost/test', 'db', {
    replset: null,
    mongos: true
  }, function (err) {}).then(cb).then(null, cb);
  conn1.close().then(null, function (err) {});
  ...
  querycursor.eachAsync(function (doc) {
    doc.execPopulate();
  }, function (err) {}).then(null, cb);
  querycursor.next(cb).then(null, cb);
  /* inherited properties */
  querycursor.pause();
  ...
  doc.validate().then(null).then(null, null);
  ...
  doc.populate(cb);
  doc.populate({path: 'hello'}).execPopulate().then(null, cb);
  ...
  /* practical examples */
  interface IManagerSchema extends mongoose.MongooseDocument {
  ...
  query.then(null, function (err) {}).then(null, null);
  query.center({}).center({});
  ...
  aggregate.exec().then(cb).then(null, cb);
  aggregate.explain(cb).then(cb).then(null, cb);
  ...
  aggregate.then(cb).then(null, cb);
  ...
  mongopromise.then(null, function (err: any) {
    err.stack;
  }).then(null, function () {});
  mongopromise.end();
  ...
  MongoModel.findById(999, function (err, doc) {
    doc.increment();
    doc.save(cb).then(cb).then(null, cb);
  });
  ...
  mongoModel.save().then(function (product) {
    product.save().then(cb).then(null, cb);
  });
  ...
  MongoModel.remove({_id: '999'}).exec().then(cb).then(null, cb);
  ...
  var Final: MyModel = <MyModel>mongoose.connection.model<MyDocument>('Final', mySchema);
